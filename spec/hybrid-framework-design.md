...
<prompt>
  ...
  <todo>
    ...
    {{#if spec_files}}Step N: Read specification files: {{spec_files as @-prefixed list}}. {{/if}}
    {{#if guideline_files}}Step N: Read guideline files: {{guideline_files as @-prefixed list}}. {{/if}}
    {{#if pr_number}}Step N: Fetch and review PR #{{pr_number}} diff. {{#if lenses}} Focus on: [{{lenses}}]. {{/if}} {{/if}}
    {{#if files}}Step N: Read and analyze {{files as @-prefixed list}}. {{#if lenses}} Focus on: [{{lenses}}]. {{/if}} {{/if}}
...
<prompt>
  ...
  <todo>
    ...
    {{#if panel_a.files}}Step N: Read existing files to build upon: {{files as @-prefixed list}}. {{/if}}
    {{#if spec_files}}Step N: Read specification files: {{spec_files as @-prefixed list}}. {{/if}}
    Step N: Create new branch. {{#if branch_name}} Name it {{branch_name}}. {{/if}}
...
<prompt>
  ...
  <todo>
    ...
    {{#if files}}Step N: Read files to improve: {{files as @-prefixed list}}. {{/if}}
    {{#if issue_a}}Step N: Read issue #{{issue_a}} for current state context. {{/if}}
    {{#if issue_b}}Step N: Read issue #{{issue_b}} for desired state. {{/if}}
    {{#if guideline_files}}Step N: Read reference files: {{guideline_files as @-prefixed list}}. {{/if}}
    Step N: Create new branch. {{#if branch_name}} Name it {{branch_name}}. {{/if}}
    Step N: Apply improvements. {{#if lenses}} Focus on: [{{lenses}}]. {{/if}}
    Step N: Verify improvements meet the desired outcome.
    Step N: Commit changes and open PR. {{#if pr_name}} Name it {{pr_name}}. {{/if}}
    Step N: Provide concise feedback to HUMAN (me) here (in this interface) include:
              - Summary of improvements made in one sentence.
