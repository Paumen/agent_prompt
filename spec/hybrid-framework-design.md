# Hybrid Framework — Full Design (v3)
...
8. [if improve_scope == "each_file"] Apply improvements to each file independently
...
All flows follow a unified structure with `<current_state>` (Situation) and `<expected_outcome>` (Target) sections.
...
<prompt>
  ...
  <todo>
    ...
    {{#if files}}Step 2: Read {{files as @-prefixed list}}{{/if}}
    {{#if issue}}Step 3: Read issue #{{issue_number}}{{/if}}
    {{#if spec_files}}Step 4: Read {{spec_files as @-prefixed list}}{{/if}}
    {{#if guideline_files}}Step 5: Read {{guideline_files as @-prefixed list}}{{/if}}
    Step 6: Identify the root cause — focus on [{{lenses}}]
    Step 7: Create new branch
    Step 8: Implement fix — focus on [{{lenses}}]
    Step 9: Run tests
    Step 10: Commit changes and open PR
  </todo>
</prompt>
...
<prompt>
  ...
  <todo>
    Step 1: Read @claude.md
    {{#if spec_files}}Step 2: Read {{spec_files as @-prefixed list}}{{/if}}
    {{#if guideline_files}}Step 3: Read {{guideline_files as @-prefixed list}}{{/if}}
    {{#if pr}}Step 4: Fetch and review PR #{{pr_number}} diff — focus on [{{lenses}}]{{/if}}
    {{#if files}}Step 5: Read and analyze {{files as @-prefixed list}} — focus on [{{lenses}}]{{/if}}
    Step 6: Provide structured feedback with file/line references
  </todo>
</prompt>
<notes>{{user_text}}</notes>
```
...
<prompt>
  ...
  <todo>
    Step 1: Read @claude.md
    {{#if panel_a.files}}Step 2: Read {{files as @-prefixed list}}{{/if}}
    {{#if spec_files}}Step 3: Read {{spec_files as @-prefixed list}}{{/if}}
    Step 4: Create new branch
    Step 5: Implement requirements — focus on [{{lenses}}]
    {{#if acceptance_criteria}}Step 6: Verify acceptance criteria{{/if}}
    Step 7: Run tests
...
<prompt>
  ...
  <todo>
    Step 1: Read @claude.md
    {{#if files}}Step 2: Read {{files as @-prefixed list}}{{/if}}
    {{#if issue_a}}Step 3: Read issue #{{issue_a}}{{/if}}
    {{#if issue_b}}Step 4: Read issue #{{issue_b}}{{/if}}
    {{#if guideline_files}}Step 5: Read {{guideline_files as @-prefixed list}}{{/if}}
    Step 6: Create new branch
    Step 7: Apply improvements — focus on [{{lenses}}]
    Step 8: Verify improvements
    Step 9: Commit changes and open PR
  </todo>
</prompt>
<notes>{{user_text}}</notes>
```
...
### Unified Structure Across All Flows

All four flows now use the same structural pattern:

```xml
<prompt>
  <context>
    <!-- Repository, branch, PAT — identical across all flows -->
  </context>
  <task flow="{{flow_id}}">
    <current_state>
      <!-- Situation panel content — what exists / what's being worked on -->
    </current_state>
    <expected_outcome>
      <!-- Target panel content — what's desired / criteria for success -->
    </expected_outcome>
    <!-- Optional: flow-specific elements like <scope> for improve -->
  </task>
  <todo>
    <!-- Auto-generated steps with conditional content -->
  </todo>
</prompt>
<notes>
  <!-- Optional user notes -->
...
| Section | Panel | Semantic Meaning |
|---------|-------|------------------|
| `<current_state>` | Panel A (Situation) | What exists, what's broken, what to review, what to improve |
| `<expected_outcome>` | Panel B (Target) | What should happen, criteria, requirements, desired state |

### Benefits of Unified Structure

1. **Consistency for LLM**: Claude receives the same structural cues regardless of flow type
2. **Clearer Mental Model**: Users understand "where we are" → "where we want to be"
3. **Simpler Template Logic**: One template structure with conditional content
4. **Better Debugging**: Easier to identify missing information when structure is predictable
...
- **New**: All templates use unified `<current_state>` / `<expected_outcome>` structure
...
| 4   | Prompt structure        | Unified: `<current_state>` / `<expected_outcome>` across all flows |
