...
// Picker field icons
const ICON_PR = `<svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" aria-hidden="true" style="display:block;flex-shrink:0"><path d="M1.5 3.25a2.25 2.25 0 1 1 3 2.122v5.256a2.251 2.251 0 1 1-1.5 0V5.372A2.25 2.25 0 0 1 1.5 3.25Zm5.677-.177L9.573.677A.25.25 0 0 1 10 .854V2.5h1A2.5 2.5 0 0 1 13.5 5v5.628a2.251 2.251 0 1 1-1.5 0V5a1 1 0 0 0-1-1h-1v1.646a.25.25 0 0 1-.427.177L7.177 3.427a.25.25 0 0 1 0-.354ZM3.75 2.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm0 9.5a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Zm8.25.75a.75.75 0 1 0 1.5 0 .75.75 0 0 0-1.5 0Z"/></svg>`;

const ICON_ISSUE = `<svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" aria-hidden="true" style="display:block;flex-shrink:0"><path d="M8 9.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"/><path d="M8 0a8 8 0 1 1 0 16A8 8 0 0 1 8 0ZM1.5 8a6.5 6.5 0 1 0 13 0 6.5 6.5 0 0 0-13 0Z"/></svg>`;

const ICON_FILE = `<svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" aria-hidden="true" style="display:block;flex-shrink:0"><path d="M2 1.75C2 .784 2.784 0 3.75 0h6.586c.464 0 .909.184 1.237.513l2.914 2.914c.329.328.513.773.513 1.237v9.586A1.75 1.75 0 0 1 13.25 16h-9.5A1.75 1.75 0 0 1 2 14.25Zm1.75-.25a.25.25 0 0 0-.25.25v12.5c0 .138.112.25.25.25h9.5a.25.25 0 0 0 .25-.25V6h-2.75A1.75 1.75 0 0 1 9 4.25V1.5Zm6.75.062V4.25c0 .138.112.25.25.25h2.688l-.011-.013-2.914-2.914-.013-.011Z"/></svg>`;
...
function renderPanelFields(panelEl, fieldsMap, panelKey) {
  ...
  // Required group unsatisfied indicator (one per group)
  for (const groupKey of Object.keys(requiredGroups)) {
    const indicator = document.createElement('div');
    indicator.className = 'required-group-unsatisfied';
    indicator.dataset.groupKey = `${panelKey}.${groupKey}`;
    indicator.textContent =
      'At least one of the required fields above must be filled.';
...
function updateRequiredGroupIndicators() {
  ...
  // Find all required group unsatisfied indicators
  const indicators = elPanelArea.querySelectorAll(
    '.required-group-unsatisfied'
  );
  for (const ind of indicators) {
    const groupKey = ind.dataset.groupKey; // e.g., "panel_a.a_required"
    const panelKey = groupKey.split('.')[0]; // e.g., "panel_a"

    // Find all fields belonging to this required group in the current flow
    ...
    const groupName = groupKey.split('.').slice(1).join('.'); // e.g., "a_required"
    const groupFields = Object.entries(panelDef.fields)
      .filter(([, fDef]) => fDef.required_group === groupName)
      .map(([fName]) => fName);

    // Check if at least one field in the group is filled
