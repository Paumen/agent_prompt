...
```
spec_concept.md  >  flows.yaml  >  source code
```
...
2. After implementing a spec requirement, update its status in the Implementation Status table at the bottom of `spec_concept.md` (`pending` → `implemented` → `tested`).
3. Log significant technical decisions in the Decisions Log section of `spec_concept.md`.
...
7. If you encounter a merge conflict in `spec_concept.md`, resolve by keeping ALL status updates AND ALL spec content — never discard either side's changes.
